...
export const SuppliersView = ({ data, onSave }: any) => {
    ...
    const toggleStatus = (supplier: any) => {
        ...
        onSave({ ...supplier, status: newStatus }, true);
    };
...
const InfractionsView = ({ data, onSave }: any) => {
  ...
  const handleSaveInfraction = () => {
      ...
      onSave({...newInfraction, id, status: 'aguardando_defesa'}, false);
      setShowModal(false);
  }
        ...
        <Modal isOpen={!!selectedInfraction} onClose={() => setSelectedInfraction(null)} title="Detalhes da Infração">
            <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-2">
                    <button onClick={() => handleStatusChange('aguardando_defesa')} className="py-2 bg-amber-100 text-amber-700 rounded hover:bg-amber-200 text-sm font-medium">Aguardando Defesa</button>
    ...
    <div className="space-y-6">
        ...
        <Modal isOpen={docToDelete !== null} onClose={() => setDocToDelete(null)} title="Confirmar Exclusão">
            ...
            <div className="flex justify-end gap-2">
                <button type="button" onClick={() => setDocToDelete(null)} className="px-4 py-2 text-slate-600 hover:bg-slate-100 rounded-lg">Cancelar</button>
                <button type="button" onClick={handleDelete} className="px-4 py-2 bg-rose-600 text-white rounded-lg hover:bg-rose-700">Excluir</button>
            </div>
        </Modal>
    </div>
  );
...
const SettingsView = ({ users, onUpdateUsers, condos, currentCondoId, onUpdateCondo, settings, onUpdateSettings }: any) => {
  ...
  const [settingsForm, setSettingsForm] = useState(settings || {
      ...
      events: { infractions: true, maintenance: true, finance: true, docs: true }
  });
        ...
        {activeTab === 'notifications' && (
            <Card className="p-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div>
                        <h4 className="text-xs font-bold text-slate-500 uppercase mb-3">Canais</h4>
                        <div className="space-y-3">
                            <label className="flex items-center gap-2">
                                <input type="checkbox" checked={settingsForm.emailNotifications} onChange={e => setSettingsForm({...settingsForm, emailNotifications: e.target.checked})} className="rounded text-indigo-600 focus:ring-indigo-500"/>
                                <span className="text-slate-700">Notificações por Email</span>
                            </label>
                            <label className="flex items-center gap-2">
                                <input type="checkbox" checked={settingsForm.pushNotifications} onChange={e => setSettingsForm({...settingsForm, pushNotifications: e.target.checked})} className="rounded text-indigo-600 focus:ring-indigo-500"/>
                                <span className="text-slate-700">Notificações no App (Push)</span>
                            </label>
                        </div>
                    </div>
                    <div>
                        <h4 className="text-xs font-bold text-slate-500 uppercase mb-3">Tipos de Evento</h4>
                        <div className="space-y-3">
                            <label className="flex items-center gap-2">
                            ...
                            <label className="flex items-center gap-2">
                                ...
                                <span className="text-slate-700">Contas a Pagar Vencendo</span>
                            </label>
                            <label className="flex items-center gap-2">
                                <input type="checkbox" checked={settingsForm.events.docs} onChange={e => setSettingsForm({...settingsForm, events: {...settingsForm.events, docs: e.target.checked}})} className="rounded text-indigo-600 focus:ring-indigo-500"/>
                                <span className="text-slate-700">Documentos e Alvarás Expirando</span>
                            </label>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex justify-end mt-8">
                    <button onClick={handleSaveSettings} className="bg-indigo-600 text-white px-6 py-2 rounded-lg font-bold hover:bg-indigo-700 shadow-lg shadow-indigo-200">Salvar Preferências</button>
                </div>
            </Card>
        )}
...
const App = () => {
  ...
  const [appSettings, setAppSettings] = useState(() => load('settings', {
      alertDays: 7,
      emailNotifications: true,
      pushNotifications: true,
      events: { infractions: true, maintenance: true, finance: true, docs: true }
  ...
  const notifications = useMemo(() => {
      const alerts = [];
      const days = appSettings.alertDays || 7;
      
      // Document Alerts
      if (appSettings.events.docs) {
          const expiredDocs = documents.filter((d:any) => {
              if (d.condoId !== currentCondoId || d.permanent) return false;
              const diffDays = Math.ceil((new Date(d.validUntil).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24));
              return diffDays <= days;
          }).length;
          if(expiredDocs > 0) alerts.push({ id: 1, text: `${expiredDocs} documento(s) vencendo ou vencidos.`, type: 'alert' });
      }

      // Finance Alerts
      if (appSettings.events.finance) {
          const pendingBills = finance.filter((f:any) => {
              if (f.condoId !== currentCondoId || f.status === 'paid' || f.type !== 'expense') return false;
              const diffDays = Math.ceil((new Date(f.dueDate).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24));
              return diffDays <= days;
          }).length;
          if(pendingBills > 0) alerts.push({ id: 2, text: `${pendingBills} conta(s) a pagar vencendo em breve.`, type: 'warning' });
      }
